<?php
/**
 * Entity insertion page for BikeStores Chef employees.
 *
 * Allows Chef employees to dynamically add a product, brand, category, stock, store, or employee via an interactive form.
...
<script>
    $(document).ready(function() {
        ...
        if (add == "product") {
            ...
            $('#back-link').attr('href', 'index.php?action=GestionEC&click=employee');
            ...
            $.ajax({
                ...
                success: function(store) {
                    $("#store_id").append(`<option value="${store[0].store_id} disabled selected">${store[0].store_name}</option>`);
                }
            });
            $(".form").append(`
            ...
            if (submit) {
                submit.addEventListener("click", function() {
                    $.ajax({
                        url: `https://bikestoresab.alwaysdata.net/bikestores/api.php?actionPost=insertEmployee&store=${<?php echo $_SESSION["StoreEmployee"] ?>}&name=${$("#employee_name").val()}&email=${$("#employee_email").val()}&password=${$("#employee_password").val()}&role=employee&KEY=e8f1997c763`,
                        type: "POST",
                        dataType: "json",
                        success: function(response) {
                            if (response.error) {
